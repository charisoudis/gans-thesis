...
class ManifoldEstimator:
    ...
    def evaluate(self, eval_embeddings: Tensor) -> Tensor:
        ...
        return torch.from_numpy(batch_predictions).mean(dim=1)
...
class F1(FID):
    ...
    def __init__(self, chkpts_root: str = '/home/achariso/PycharmProjects/gans-thesis/.checkpoints',
                 device: str = 'cpu', n_samples: int = 1024, batch_size: int = 8):
        """
    ...
    def forward(self, dataset: Dataset, gen: nn.Module, target_index: Optional[int] = None,
                ...
                k: int = 3, row_batch_size: int = 8, col_batch_size: int = 8) -> Tuple[Tensor, Tensor, Tensor]:
        """
        ...
        real_embeddings, fake_embeddings = self.get_embeddings(dataloader, gen, target_index, condition_indices, z_dim)
        ...
        precision = real_manifold.evaluate(fake_embeddings)
        print(precision)

        # Compute recall (i.e. how many points from real_embeddings are in fake_embeddings manifold)
        recall = fake_manifold.evaluate(real_embeddings)
        print(recall)
